...
import {
  ...
  listMentionDeliveries, requeueMentionDelivery, processMentionQueueNow,
...
export function BackgroundQueuesCard() {
  ...
  const requeueFn = useServerFn(requeueMentionDelivery);
  ...
  const requeueMut = useMutation({
    mutationFn: (deliveryId: string) => requeueFn({ data: { deliveryId } }),
                    ...
                    <TableCell>
                      {canRetry && (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => requeueMut.mutate(d.id)}
