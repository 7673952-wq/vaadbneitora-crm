...
## נוהל בטיחות למיגרציות הרשאות

כל מיגרציה שנוגעת ב-`GRANT` / `REVOKE` / `RLS` / `CREATE|ALTER|DROP POLICY` /
`GRANT EXECUTE` / `SECURITY DEFINER` על אובייקט קיים חייבת לעבור לפני production:

1. הרצה על מסד נקי (סכימה מלאה מאפס).
2. הרצה על עותק שמדמה את המצב הקיים בפרודקשן.
3. Smoke test עם משתמש רגיל (לא מנהל): רשימת מערכות, פתיחת כרטיס, כתיבת הערה,
   שמיעת הקלטה של בקשה, שליחת מייל.
4. בדיקה שפעולות שאמורות להיחסם אכן נחסמות (מחיקת מערכת ללא הרשאה, שינוי הרשאות).
5. בדיקות חוצות-CRM: משתמש של CRM אחד לא רואה ולא משנה נתונים של CRM אחר.
6. אין להרחיב הרשאות (`USING (true)`, `GRANT` גורף) רק כדי שהממשק יעבוד.

מיגרציה שכבר הופעלה אינה נערכת — תיקון נעשה במיגרציה חדשה בלבד.

## החרגת סודות מארכיונים

ארכיוני שיתוף/גיבוי קוד לא כוללים לעולם: `.env`, `.env.*`, קבצי credentials,
קבצי secrets וקבצי הרשאה זמניים. מותר לכלול `.env.example` עם ערכי דמה בלבד.
