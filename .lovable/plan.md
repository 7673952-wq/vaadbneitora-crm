...
### נרמול זהה בין TypeScript ל-SQL (ההערה נכונה — הוגדר במפורש)
הגדרה יחידה: **ספרות בלבד, אפסים מובילים נשמרים** (`0527673952` נשאר כמות שהוא), ללא חיתוך, ללא היפוך וללא התאמה משוערת.
- TS: `normalizeSystemCode(s) = String(s).replace(/\D/g, "")`
- SQL: `regexp_replace(system_code, '\D', '', 'g')` — סמנטיקה זהה, כולל שמירת אפסים מובילים.
- בדיקה אוטומטית: טסט שמריץ סט ערכים (רווחים, מקפים, אפסים מובילים, יוניקוד, ריק) דרך שתי המימושים — TS מקומית ו-SQL דרך שאילתה — ומוודא ערך זהה. הטסט ירוץ בכל בנייה, כדי שהאינדקס והחיפוש לעולם לא יתפצלו.

## 11. תוכנית בדיקות
בדיקות יחידה: נרמול מספר מערכת (כולל השוואת TS מול SQL), פרסור נושא+גוף כולל פורמטים פגומים, מנוע כללים (כלל מדויק גובר על ברירת מחדל; אין כלל → `needs_decision`), כשירות שליחה קולית (A→B→A לא שולח A פעמיים). בדיקות אינטגרציה: אותו payload פעמיים ושלוש פעמים, קריסה מדומה בכל שלב (כולל אחרי ה-RPC), שתי בקשות במקביל לאותו מספר מערכת, ואימות ש-DRY RUN לא כתב אף שינוי תפעולי. בדיקות ידניות: DRY RUN על מיילים אמיתיים והשוואת ההחלטה המוצעת מול החלטת אדם.
