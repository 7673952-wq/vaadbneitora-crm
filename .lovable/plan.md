...
|---|---|---|
| `report_description` — parser עם `:` | OPEN — ממצא מוכח | `FIELD_LABEL_LINE` בשרת ו־`messageReportDescription_` ב־Apps Script עוצרים על כל שורה שנראית `טקסט:`; ב־Apps Script גם `12:30` עוצר. תיקון בשני הצדדים: עצירה רק על רשימת כותרות השדה האמיתיות של פורמט המייל (מספר בקשה/מערכת/פונה/טלפון/תאריך/סוג בקשה/מפריד `---`). בדיקות: נקודתיים במשפט, שעה `12:30`, כמה שורות, שורה שנראית ככותרת אך אינה שדה מוכר, כותרת שדה אמיתית אחריה. |
...
| `voice_queue_url` deployment | OPEN | הכתובת מוגדרת במסד, אך אין למנהל תצוגה/הגדרה/health check. אותו מנגנון כמו תור התיוגים (סעיף 7): כרטיס משותף "תורי רקע" בניהול לשני התורים — endpoint נוכחי, "הגדר לסביבה זו" מ־`APP_BASE_URL` (לא Host), health check עם token, זיהוי endpoint חסר/שגוי, תיעוד staging/production. תרחיש החובה (pending → dashboard סגור → cron → endpoint → נשלח → ריק → disarm) — אם לא הורץ בפועל: NOT RUN + NOT READY FOR LIVE. |
...
| מייל רגיל: relay הצליח + DB/תהליך נכשל | OPEN | `sendSystemEmail`/`sendRecordEmail`/`sendMailboxMessage` פונים ל־relay ורק אחר כך כותבים ל־DB; קריסה או כשל אחרי הצלחת relay → לחיצה חוזרת שולחת שוב. תיקון: טבלת outbox `email_deliveries` (service_role בלבד) עם `idempotency_key` ייחודי. המפתח נוצר פעם אחת לכל send intent ונשמר ב־`sessionStorage` תחת מפתח ההקשר (מערכת/רשומה/שרשור + מצב compose) עד להצלחה/כשל סופי — רענון או פתיחה מחדש של אותו חלון משתמשים באותו מפתח; "הודעה חדשה" ביוזמת המשתמש מייצרת intent חדש במפורש. לפני הפנייה ל־relay השרת יוצר/מאתר operation לפי המפתח; מצבים `pending → sending → sent | failed | unknown`. הרשומה נכתבת **לפני** הפנייה ל־relay; מפתח קיים במצב `sending/sent/unknown` → אין פנייה נוספת לספק, מוחזר המצב הקיים. הצלחת relay + כשל finalization → `unknown` + הודעה למשתמש "ייתכן שנשלח — בדוק בתיבה"; אין resend אוטומטי. `sending` ישן (מעל 10 דק') → `unknown`. |
...
- dedup אחרי מחיקה: מפתחות ה־dedup (`gmail_message_id` UNIQUE, `system_requests_dedupe_uniq`) לא נמחקים. סריקה חוזרת של אותה הודעת Gmail מוצאת את השורה המחוקה ומחזירה `duplicate/deleted` בלי ליצור בקשה חדשה ובלי לעבד אותה מחדש; כפילות עסקית (אותו מספר בקשה) עדיין נופלת על האינדקס ומסומנת `duplicate_of` על השורה המחוקה — דטרמיניסטי. שחזור רק בפעולה מפורשת `restoreSystemRequest` (הרשאת `requests_delete`, rate limit). בדיקות: soft-delete → rescan → אין בקשה חדשה; soft-delete → כפילות עסקית → אותה תוצאה כמו קודם.
...
- `profiles` — effective privileges מלאים (נבדק): `authenticated` מחזיק SELECT/INSERT/UPDATE/DELETE ברמת טבלה, ולכן ה־grants ברמת עמודות (`id, display_name, email_display_name, created_at`) חסרי משמעות בפועל — כל חבר קורא גם `email_signature` של אחרים. מסלולי הקוד: קריאות משתמש רק ל־`id, display_name` (ועמודות החתימה של עצמו ב־`email.functions.ts`); כל הכתיבות המנהליות דרך service_role; יצירת פרופיל דרך טריגר `handle_new_user`; אין INSERT/DELETE מהלקוח. לכן באותה מיגרציה: REVOKE ALL ברמת טבלה מ־`authenticated`; GRANT SELECT (`id, display_name, email_display_name, created_at`) ברמת עמודות; ללא GRANT UPDATE כלל: הכתיבה הישירה היחידה מ־client מאומת היא `email_signature` ב־`email.functions.ts:139`, והיא עוברת ל־service_role אחרי אימות `context.userId`; `display_name`/`email_display_name` כבר נכתבים רק דרך service_role. effective privilege מינימלי: SELECT על 4 עמודות בלבד. המדיניות `insert_self`/`delete_admin` נשארות אך ללא grant — לא פעילות. אין שינוי בממשק למנהלים.
