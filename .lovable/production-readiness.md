...
5. **Pagination ל-`listSystems`** — ה-API מחזיר את כל המערכות. מתאים
   לטבלאות קטנות-בינוניות, אך עם 5000+ מערכות יהיו תקלות ביצועים.
   ה-Schema של `listSystems` מוכן להרחבה — להוסיף `page`/`pageSize` ולעבור
   ל-`range()`.
